...
void LibNorm_MostValue()
{
	...
	cout << x.max() << endl;
	cout << x.amax(0).unsqueeze(0) << endl;
	cout << x.amin(0).unsqueeze(0) << endl;
	cout << std::get<0>(x.max(0)).unsqueeze(0) << endl;		// return maxs values
	cout << std::get<1>(x.max(0)).unsqueeze(0) << endl;		// return maxs index
}
